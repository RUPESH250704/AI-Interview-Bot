...
def upload_resume():
    ...
    if difficulty == 'easy':
        tone = f"Ask ONE simple technical question about skills from your resume. Be friendly. Max 10 words. Skills: {resume_content[:500]}"
    elif difficulty == 'hard':
        tone = f"Ask ONE challenging technical question about skills from your resume. Be strict. Max 10 words. Skills: {resume_content[:500]}"
    else:
        tone = f"Ask ONE technical question about skills from your resume. Max 10 words. Skills: {resume_content[:500]}"
...
def chat_endpoint():
    ...
    if session['sub_question_count'] < 2:
        ...
        prompt = f"Ask a follow-up question about: '{user_message}'. Max 8 words."
    else:
...
def get_next_main_question():
    ...
    if session['question_count'] < 2:
        session['current_category'] = 'resume'
        if difficulty == 'easy':
            prompt = f"Ask a different simple technical question about skills from your resume. Be friendly. Max 8 words. Skills: {session['resume_content'][:500]}"
        elif difficulty == 'hard':
            prompt = f"Ask a challenging technical question about skills from your resume. Be strict. Max 8 words. Skills: {session['resume_content'][:500]}"
        else:
            prompt = f"Ask a different technical question about skills from your resume. Max 8 words. Skills: {session['resume_content'][:500]}"
    elif session['question_count'] < 4:
        session['current_category'] = 'company'
        if difficulty == 'easy':
            prompt = f"Ask friendly technical question about {session['company']} technology. Max 8 words."
        elif difficulty == 'hard':
            prompt = f"Ask challenging technical question about {session['company']} advanced systems. Max 8 words."
        else:
            prompt = f"Ask technical question about {session['company']} technology stack. Max 8 words."
    else:
        session['current_category'] = 'role'
        if difficulty == 'easy':
            prompt = f"Ask friendly question about {session['role']} role. Max 8 words."
        elif difficulty == 'hard':
            prompt = f"Ask challenging question about advanced {session['role']} skills. Max 8 words."
        else:
            prompt = f"Ask about {session['role']} role responsibilities. Max 8 words."
